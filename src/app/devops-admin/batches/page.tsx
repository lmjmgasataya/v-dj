            ...
            <tbody className="divide-y divide-gray-100">
              {all.map((b) => (
                <tr key={b.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2.5 text-gray-500">{b.classStartDate} – {b.classEndDate}</td>
                  <td className="px-4 py-2.5 text-gray-500">{b.registrationStartDate ?? "—"}</td>
                  <td className="px-4 py-2.5 text-center">
                    {b.isDefault ? (
                      <span className="text-xs px-2 py-0.5 rounded font-medium bg-green-100 text-green-700">Yes</span>
                    ) : (
                      <form action={setDefaultBatch}>
                        <input type="hidden" name="id" value={b.id} />
                        <button type="submit" className="text-xs px-2 py-0.5 rounded font-medium bg-gray-100 text-gray-400 hover:bg-gray-200 transition">Set</button>
                      </form>
                    )}
                  </td>
                  <td className="px-4 py-2.5">
                    <div className="flex items-center justify-end gap-2">
                      <Link href={`/devops-admin/batches/${b.id}/edit`} className="text-xs text-indigo-600 hover:text-indigo-800 font-medium">Edit</Link>
                      <ConfirmDeleteButton
                        action={deleteBatch}
