...
import { addLeadership113Batch, deleteLeadership113Batch } from "./actions";
...
function Row({ batch }: { batch: Leadership113Batch }) {
  ...
  function handleDelete() {
    ...
    startTransition(() => deleteLeadership113Batch(batch.id));
  }

  return (
    <tr className="hover:bg-gray-50">
      <td className="px-4 py-2.5 text-gray-700">{batch.batchName}</td>
      <td className="px-4 py-2.5 text-gray-700 font-semibold">{batch.actual}</td>
      <td className="px-4 py-2.5 text-gray-500">{batch.goal}</td>
      <td className="px-4 py-2.5">
        {retention == null ? (
          "—"
        ) : (
          <span className={retention >= 100 ? "text-green-600 font-semibold" : "text-amber-600 font-semibold"}>
            {retention}%
          </span>
        )}
      </td>
...
export function Leadership113Section({ batches }: { batches: Leadership113Batch[] }) {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      await addLeadership113Batch(formData);
      setAdding(false);
    });
  }
      ...
      {adding && (
        <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-3 gap-3 p-4 border-t border-gray-100 bg-gray-50">
          <div>
            ...
            <input name="batchName" required placeholder="e.g. Batch 9" className={inputCls} />
          </div>
          <div>
            <label className="block text-xs text-gray-500 mb-1">Actual</label>
            <input name="actual" type="number" min={0} required className={inputCls} />
          </div>
          <div>
            <label className="block text-xs text-gray-500 mb-1">Goal</label>
            <input name="goal" type="number" min={0} required className={inputCls} />
          </div>
