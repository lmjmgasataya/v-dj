...
export async function registerParticipant(formData: FormData) {
  ...
  if (isVGLeaderFlow) {
    ...
    await Promise.all([
      ...
      db.insert(victoryGroupLeaders).values({
        lastName: vgLeaderLastName,
        firstName: vgLeaderFirstName,
        mobileNumber: vgLeaderMobileNumber,
        facebookMessengerName: vgLeaderMessengerName,
      }).onConflictDoNothing(),
    ]);
  } else {
