...
function GroupRow({ group }: { group: VictoryGroup }) {
  ...
  function handleDelete() {
    ...
    startDeleteTransition(() => deleteOwnVictoryGroup(group.id));
  }
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
      <div className="bg-indigo-50 border-b border-indigo-100 px-6 py-3 flex items-center justify-between">
        <h2 className="text-sm font-semibold text-indigo-800 uppercase tracking-wide">My Victory Groups</h2>
        {!adding && (
          <button
      ...
      <div className="p-4 flex flex-col gap-3">
        ...
        {adding && (
          <GroupForm
            onSave={(formData) => addOwnVictoryGroup(formData)}
